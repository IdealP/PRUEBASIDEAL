...
$sentencia = $base_de_datos->prepare("UPDATE pagosmenc SET nombre_escuela = ?, sigla_escuela = ?, tramites = ?, retro = ?, costo = ?, total = ?, fecha_ingreso = ?, restante = ? WHERE nombre_escuela = ?;");
...
if($resultado === TRUE){
	header("Location: ./listaMesCPM.php");
	exit;
}
